...
import { vanBrandsCategory } from '@/utils/vehicles/vans/vanBrands';
...
import { daihatsuModelsCategory } from '@/utils/vehicles/vans/daihatsu';
import { hondaModelsCategory } from '@/utils/vehicles/vans/honda';
import { hyundaiModelsCategory } from '@/utils/vehicles/vans/hyundai';
import { isuzuModelsCategory } from '@/utils/vehicles/vans/isuzu';
import { mazdaModelsCategory } from '@/utils/vehicles/vans/mazda';
import { mahindraModelsCategory } from '@/utils/vehicles/vans/mahindra';
import { mercedesModelsCategory } from '@/utils/vehicles/vans/mercedes';
import { microModelsCategory } from '@/utils/vehicles/vans/micro';
import { mitsubishiModelsCategory } from '@/utils/vehicles/vans/mitsubishi';
import { nissanModelsCategory } from '@/utils/vehicles/vans/nissan';
import { subaruModelsCategory } from '@/utils/vehicles/vans/subaru';
import { suzukiModelsCategory } from '@/utils/vehicles/vans/suzuki';
import { tataModelsCategory } from '@/utils/vehicles/vans/tata';
import { toyotaModelsCategory } from '@/utils/vehicles/vans/toyota';
...
export default  function  VanSparesComponent(){
    ...
    const defaultCategory=`${process.env.VANPARTS_ID}`
    const [myFilters, setMyFilters] = useState({
        filters: { category: [defaultCategory], price: [],vanBrand:[],district:[],localArea:[] }
    });
...
const daihatsuModels=()=>(
    <div>
        ...
        <ul>
            <CarCheckBox
                ...
                handleFilters(filters, "vanModel")
                }
            />
        </ul>
        </div>
   ...
   const mercedesModels=()=>(
    <div>
        <h4>select mercedes benz model</h4>
        <div className="default-checkbox-div">
        <ul>
            <CarCheckBox
                categories={mercedesModelsCategory}
                handleFilters={filters =>
                handleFilters(filters, "vanModel")
                }
            />
      ...
      <div>
              ...
              <div className="row">
                ...
                <h4 className="my-3 "><span class="badge text-bg-secondary">Filter by District</span></h4>
                    ...
                    {
                        (myFilters.filters.vanBrand.indexOf('daihatsu')!=-1) ? daihatsuModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('honda')!=-1) ? hondaModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('hyundai')!=-1) ? hyundaiModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('isuzu')!=-1) ? isuzuModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('mahindra')!=-1) ? mahindraModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('mazda')!=-1) ? mazdaModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('mercedes benz')!=-1) ? mercedesModels():(myFilters.filters.vanModel=[])
                    }{
                        (myFilters.filters.vanBrand.indexOf('micro')!=-1) ? microModels():(myFilters.filters.vanModel=[])
                    }{
                        (myFilters.filters.vanBrand.indexOf('mitsubishi')!=-1) ? mitsubishiModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('nissan')!=-1) ? nissanModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('subaru')!=-1) ? subaruModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('suzuki')!=-1) ? suzukiModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('tata')!=-1) ? tataModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('toyota')!=-1) ? toyotaModels():(myFilters.filters.vanModel=[])
                    }
                    {
                        (myFilters.filters.vanBrand.indexOf('mazvolksda')!=-1) ? mazdaModels():(myFilters.filters.vanModel=[])
                    }
                        ...
                        <div class="modal-dialog ">
                            ...
                            <div class="modal-body">
                                ...
                                <div className="default-checkbox-div">
                                    <ul>
                                        <CarCheckBox
                                            categories={vanBrandsCategory}
                                            handleFilters={filters =>
                                                handleFilters(filters, "vanBrand")
                                            }
                                        />
                                    </ul>
